...
import {
  ...
  TRAVELER_M_GEO_KIT_METADATA,
} from "./travelerMGeoDefinition";

const noCrit = { critMode: "never" as const };

function damages(result: ReturnType<typeof simulateRotation>) {
  return result.timeline
    .filter((event) => event.type === "damage" && event.characterId === "traveler-m-geo")
    .map((event) => event.damage!);
}

function run(constellationLevel: number, actionType: "skill" | "burst") {
  const traveler = createTravelerMGeoDefinition(constellationLevel);
...
describe("Male Traveler Geo runtime kit", () => {
  ...
  it("does not infer unverified A4 damage or unsupported C2 destruction damage", () => {
    // The C5 skill talent boost is verified; C6 adds no modeled damage instance.
    expect(damages(run(5, "skill")).map((hit) => hit.finalDamage)).toEqual(
      damages(run(6, "skill")).map((hit) => hit.finalDamage),
    );
    expect(TRAVELER_M_GEO_KIT_METADATA.unsupportedChannels).toContain(
